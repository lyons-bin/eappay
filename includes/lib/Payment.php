                    ...
                    if(!empty($channel['subid'])){
                        if($DB->exec("UPDATE pre_subchannel SET status=0 WHERE id='{$channel['subid']}'")){
                            if($conf['check_paymsg_notice'] == 1){
                                ...
                                send_mail($mail_name,$conf['sitename'].' - 支付通道自动关闭提醒','尊敬的管理员：支付通道“'.$channel['name'].'”下的子通道“'.$channel['subname'].'”因用户下单时出现异常提示“'.$msg.'”，已被系统自动关闭！<br/>----------<br/>'.$conf['sitename'].'<br/>'.date('Y-m-d H:i:s'));
                            }
                        }
                    }else{
                        if($DB->exec("UPDATE pre_channel SET status=0 WHERE id='{$channel['id']}'")){
                            if($conf['check_paymsg_notice'] == 1){
                                $mail_name = $conf['mail_recv']?$conf['mail_recv']:$conf['mail_name'];
                                send_mail($mail_name,$conf['sitename'].' - 支付通道自动关闭提醒','尊敬的管理员：支付通道“'.$channel['name'].'”因用户下单时出现异常提示“'.$msg.'”，已被系统自动关闭！<br/>----------<br/>'.$conf['sitename'].'<br/>'.date('Y-m-d H:i:s'));
                            }
                        }
                    }
                    break;
