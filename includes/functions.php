...
function processOrder($srow,$notify=true){
	...
	if($srow['tid']==1){ //商户注册
		...
		if($upid > 0){
			...
			if($conf_n['invite_open'] == 1 && $conf_n['invite_groupbuy_rate'] > 0){
				$invite_money = round($addmoney * $conf_n['invite_apply_rate'] / 100, 2);
				if($invite_money > 0){
	...
	if($srow['profits']>0){ //订单分账处理
		...
		if($psreceiver){
			...
			$DB->insert('psorder', ['rid'=>$psreceiver['id'], 'trade_no'=>$srow['trade_no'], 'api_trade_no'=>$srow['api_trade_no'], 'money'=>$psmoney, 'status'=>$status, 'addtime'=>'NOW()', 'delay'=>$delay]);
		}
	}
}
